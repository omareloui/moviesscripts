...
export async function moveFile(src: string, dest: string): Promise<void> {
  try {
    ...
    console.error(colors.red.bold.white(`Problem moving ${src} to ${dest}.`));
...
export async function moveFolderRecursively(src: string, dest: string) {
  ...
  try {
    for await (const f of Deno.readDir(src)) {
      ...
      await Deno.rename(path.join(src, filename), path.join(dest, filename));
    }
    ...
    const message = getErrorMessage(e);
    console.error(
      colors.underline.bold.bgRed.brightBlack(`Wasn't able to move ${src}.`),
    );
